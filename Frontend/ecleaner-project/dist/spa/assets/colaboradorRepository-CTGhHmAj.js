    ...
    if (data.experiencias) {
      data.experiencias.forEach((exp) => {
        const experiencia = new ExperienciaProfissional(
          ...
          exp.Cargo || exp.cargo,
          exp.DataInicio ? new Date(exp.DataInicio) : exp.dataInicio ? new Date(exp.dataInicio) : null,
    ...
    if (data.referencias) {
      data.referencias.forEach((ref) => {
        const referencia = new Referencia(
          ...
          ref.Relacao || ref.relacao || `${ref.Cargo || ref.cargo} na ${ref.Empresa || ref.empresa}`,
...
export {
  ColaboradorRepository as C,
  colaboradorRepository as c
};
//# sourceMappingURL=colaboradorRepository-CTGhHmAj.js.map
